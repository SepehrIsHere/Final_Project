...
public class SpecialistAddTest {
    @Mock
    ...
    @Test
    void add_ShouldAddSpecialist_WhenValid() throws SpecialistOperationException {
        Specialist specialist = generateValidSpecialist();
        when(validationUtil.isValid(specialist)).thenReturn(true);

        specialistService.add(specialist);

        verify(specialistRepository, times(1)).save(specialist);
        verify(validationUtil, never()).displayViolations(specialist);
    }

    @Test
    void add_ShouldNotAddSpecialist_WhenInvalid() throws SpecialistOperationException {
        Specialist specialist = generateInvalidSpecialist();
        when(validationUtil.isValid(specialist)).thenReturn(false);

        specialistService.add(specialist);

        verify(specialistRepository, never()).save(specialist);
        verify(validationUtil, times(1)).displayViolations(specialist);
    }

    @Test
    void add_ShouldThrowSpecialistOperationException_WhenExceptionOccurs() {
        Specialist specialist = generateInvalidSpecialist();
        when(validationUtil.isValid(specialist)).thenReturn(true);
        doThrow(new RuntimeException()).when(specialistRepository).save(specialist);

        assertThrows(SpecialistOperationException.class, () -> specialistService.add(specialist));
    }
}
